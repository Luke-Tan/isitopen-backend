...
const MongoDB = {
    ...
    GetRestaurantCollections: (collectionIds) => {
        return new Promise(async (resolve,reject)=>{
            const restaurantCollectionsCollection = db.collection('RestaurantCollections');
            const mongoCollectionIds = collectionIds.map(id => mongoObjectId(id))
            try{
                restaurantCollections = restaurantCollectionsCollection.find({ _id :{$in:mongoCollectionIds}}).toArray();
    ...
    CreateRestaurantCollection: (name, restaurantId) => {
        return new Promise( async (resolve, reject)=>{
            ...
            if(restaurantId){
                const restaurantDataCollection = db.collection('RestaurantData');
                restaurant = await restaurantDataCollection.findOne({"_id": mongoObjectId(restaurantId)});
            }
            try {
    ...
    AddToRestaurantCollection: (collectionId, restaurantId) => {
        return new Promise( async (resolve, reject)=>{
            ...
            const restaurantDataCollection = db.collection('RestaurantData');
            try {
                const restaurant = await restaurantDataCollection.findOne({"_id": mongoObjectId(restaurantId)});
                const {restaurants} = await restaurantCollectionsCollection.findOne({"_id": mongoObjectId(collectionId)});
